...
- [~] Spellcasting scaffold: caster stats + save DCs + spells/day + bonus slots + prepared/known split + editable web selection UI + engine-backed selection diagnostics/warnings + capacity-aware quick-picks + selection state + count/class/level validation + slot expenditure tracking + web runtime cast controls + local persistence started; full content DB and extra-slot sources pending
