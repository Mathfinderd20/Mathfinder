...
function skillModifiers(mods: Modifier[], key: SkillKey): Modifier[] {
  return mods.filter(
    (m) =>
      m.enabled !== false &&
      (m.target === `skill.${key}` || m.target === "skill.all"),
  );
}
...
export function deriveSkills(
  ...
  for (const def of SKILL_DEFINITIONS) {
    ...
    if (ranks !== 0)
      breakdown.push({ source: "ranks", type: "base", value: ranks });
    breakdown.push({
      source: ABILITY_LABEL[def.ability],
      type: "ability",
      value: abilityMod,
    });
    ...
    result[def.key] = {
      ...
      usable:
        input.skillUsableOverrides?.[def.key] ??
        (!def.trainedOnly || ranks > 0),
      total,
      breakdown,
    };
  }
