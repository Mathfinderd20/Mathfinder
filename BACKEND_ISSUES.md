...
| BE-002 | Define initial data model for users, characters, parties, memberships, and effects | data-model   | P1       | in-progress | unassigned        | 2026-06-17 | —              | Initial gameplay schema is committed under `supabase/migrations`; Docker-backed migration execution and generated types remain.                                         |
...
| BE-005 | Define permission rules for DM vs player capabilities                              | permissions  | P1       | in-progress | unassigned        | 2026-06-17 | BE-002         | Initial RLS policies enforce player-owned builds, campaign visibility, GM metadata/invite control, and campaign runtime isolation; executable policy tests remain.      |
| BE-006 | Scaffold `services/api` package structure                                          | scaffolding  | P2       | wont-fix    | unassigned        | 2026-06-17 | —              | Supabase client + RLS/RPC/Edge Functions are the selected boundary. Add a custom Node API only when it owns behavior that those layers cannot cleanly support.          |
