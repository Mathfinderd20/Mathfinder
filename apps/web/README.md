...
Interactive Pathfinder 1e character sheet — the first UI for Mathfinder.

A Vite + React app that imports `@mathfinder/rules-engine` directly and renders
...
```bash
npm install
npm run dev --workspace @mathfinder/web      # start the dev server
npm run build --workspace @mathfinder/web    # production build
npm run typecheck --workspace @mathfinder/web
```
